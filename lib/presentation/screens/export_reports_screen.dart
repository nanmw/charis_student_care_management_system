...
class _ExportReportsScreenState extends ConsumerState<ExportReportsScreen> {
  ...
  String _selectedMode = _modeOptions.first;
  ...
  List<String> get _yearOptions => AppConstants.reportYearOptions;
      ...
      switch (_selectedReportType) {
        case ReportType.studentSummary:
          ...
          if (format == ReportFormat.pdf) {
            bytes = await ReportService.buildPdf(rows, _filters);
          } else {
            bytes = ReportService.buildExcel(rows, _filters);
          }
          break;
        case ReportType.cohortSummary:
          final rows = await ref.read(cohortReportDataProvider.future);
          if (format == ReportFormat.pdf) {
            bytes = await ReportService.buildCohortSummaryPdf(rows);
          } else {
            bytes = ReportService.buildCohortSummaryExcel(rows);
          }
          break;
  ...
  Widget build(BuildContext context) {
    ...
    if (auth is! Authenticated || !RolePermissions.canExportReports(auth.role)) {
      return Container(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                SizedBox(
                  width: 300,
                  child: _buildLeftPanel(colorScheme, redColor, isDark),
                ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    isExpanded: true,
                    key: ValueKey<String?>(_selectedYear),
                    ...
                    onChanged: (v) {
                      setState(() {
                        ...
                        if (v != null) {
                          ...
                          if (yr != null) {
                            _dateStart = DateTime(yr, 1, 1);
                            _dateEnd = DateTime(yr, 12, 31);
                          }
                        }
                      });
                    },
                  ...
                  DropdownButtonFormField<String>(
                    isExpanded: true,
                    key: ValueKey<String>(_selectedMode),
